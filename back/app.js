...
// carro-related endpoints (no authentication needed for these)
...
app.post('/carros', upload.single('image'), (req, res) => {
    const { modelo, marca, ano, valor, cor } = req.body;
    const imagePath = req.file ? req.file.path : null;
    if (!modelo || !marca || !ano || !valor || !cor || !imagePath) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    db.run(
        'INSERT INTO Carros (modelo, marca, ano, valor, cor, image) VALUES (?, ?, ?, ?, ?, ?)',
        [modelo, marca, ano, valor, cor, imagePath],
        function (err) {
